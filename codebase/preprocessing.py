def pre_process_paquete(paquete_csv, entrega_csv):
    ...
    #Preprocess .csv
    f_paquete = paquete.groupby(paquete['Fecha'].dt.strftime('%Y-%W')).size()
    
    #Group by year week
    f_paquete = pd.DataFrame(f_paquete).reset_index()
    f_paquete['Fecha'] = pd.to_datetime(f_paquete.Fecha +'-0', format='%Y-%W-%w')
    
    #Add days to week in order to convert to date
